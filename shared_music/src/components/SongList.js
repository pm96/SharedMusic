...
const Playlist = ({playlist, songDelete, changeQueueOrder_AND_playVideo}) => (
    <div className="list-container" >
        {playlist.length < 1 ? 
            ...
            {Object.entries(playlist).map(([key,item]) =>
                <ListItem key={key} id={key} title={item.snippet.title} uploader={item.snippet.channelTitle} songDelete={songDelete} changeQueueOrder_AND_playVideo={changeQueueOrder_AND_playVideo}/>
            )}
        </List>}
    </div>
)
